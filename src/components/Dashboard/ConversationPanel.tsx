...
export function ConversationPanel({ messages }: { messages: Message[] }) {
  ...
  if (messages.length === 0) {
    return (
      <div className="flex-1 flex flex-col bg-gradient-card rounded-xl shadow-card border border-slate-200 p-6 sm:p-8 mb-24 sm:mb-28 min-h-0">
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-slate-500">Press the Speak button below to tell me what you're craving</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex-1 flex flex-col bg-gradient-card rounded-xl shadow-card border border-slate-200 p-4 sm:p-6 mb-24 sm:mb-28 min-h-0">
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-4 scroll-smooth">
        {messages.map((msg, i) => (
